...
namespace GeoDo.FileProject
{
    /// <summary>
    /// 注意：
    /// 1Km数据而言，左右去除最多各去除10个，效果比较好，不能再比这个大了，如果去除过多，则在赤道地区，左右轨会出现缝隙，当然高纬度地区还可以去除更多。
    /// </summary>
    [Export(typeof(IFileProjector)), ExportMetadata("VERSION", "1")]
    internal class FY3C_MERSIFileProjector : FileProjector
    {
        ...
        private void DoSession(IRasterDataProvider srcRaster, IRasterDataProvider geoRaster, ISpatialReference dstSpatialRef, FY3_MERSI_PrjSettings prjSettings, Action<int, string> progressCallback)
        {
        ...
        private void ReadyLocations(IRasterDataProvider geoRaster, ISpatialReference dstSpatialRef, FY3_MERSI_PrjSettings fy3prjSettings,
            out Size srcLocationSize, out PrjEnvelope maxPrjEnvelope, Action<int, string> progressCallback)
        {
            ...
            srcLocationSize = new Size(_longitudeBand.Width, _longitudeBand.Height);
        ...
        protected override void ReadLocations(IRasterDataProvider geoRaster, out double[] xs, out double[] ys, out Size locationSize)
        {
            xs = null;
            ys = null;
        ...
        private void ReadLocations(IRasterDataProvider geoRaster, out IRasterBand longitudeBand, out IRasterBand latitudeBand)
        {
