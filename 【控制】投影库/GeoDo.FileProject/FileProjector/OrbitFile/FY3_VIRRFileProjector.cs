...
namespace GeoDo.FileProject
{
    ...
    internal class FY3_VIRRFileProjector : FileProjector
    {
        ...
        public override void Project(IRasterDataProvider srcRaster, FilePrjSettings prjSettings, ISpatialReference dstSpatialRef, Action<int, string> progressCallback)
        {
            try
            {
                ...
                try
                {
                    ...
                    ReadyAngleFiles(srcRaster, _outfilename, _prjSettings, _dstSize, options.ToArray());
                    ReadyExtBands(srcRaster, _outfilename, _prjSettings, _dstSize, options.ToArray());
        ...
        private void DoSession(IRasterDataProvider srcRaster, ISpatialReference dstSpatialRef, FY3_VIRR_PrjSettings prjSettings, Action<int, string> progressCallback)
        {
            if (_curSession == null || _curSession != srcRaster || _isBeginSession)
            {
                ...
                ReadyLocations(srcRaster, dstSpatialRef, _srcLocationSize, out _xs, out _ys, out _maxPrjEnvelope, progressCallback);
                if (progressCallback != null)
        ...
        private void ReadySolarZenithArgsToFile(IRasterDataProvider srcRaster)
        {
            ...
            try
            {
                ...
                //存储的是预先计算好的天顶角订正值：10/cos(SolarZenith/100/180*π),即cos(太阳天顶角弧度值),10为值放大倍数
                Parallel.For(0, length, index =>
                {
