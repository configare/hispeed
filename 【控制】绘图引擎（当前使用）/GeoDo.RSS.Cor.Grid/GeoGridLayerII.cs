...
namespace GeoDo.RSS.Core.Grid
{
    /// <summary>
    /// 类名：GeoGridLayer
    /// 属性描述：修改原有的经纬度网格绘制效率问题，只对显示区域内的网格线进行绘制
    /// 创建者：   创建日期：
    /// 修改者： 王羽            修改日期：2015-3-11 8:47:00
    /// 修改描述：
    /// 备注：
    /// </summary>
    public class GeoGridLayer : Layer, IRenderLayer, IGridLayer, IGridLayerAtrEdit
    {
        ...
        */
        ...
        //当前控件的屏幕范围，用于切割线段
        private RectangleF _curViewScrRect = RectangleF.Empty;
        //用于双线框
        private SizeF _inSize = new Size(20, 20);
        private bool _doubleBorder = false;
        private StringFormat _stringFormat = new System.Drawing.StringFormat(StringFormatFlags.DirectionVertical);

        public GeoGridLayer()
        {
            _gridPen.DashStyle = System.Drawing.Drawing2D.DashStyle.Solid;//.DashDot;
        ...
        public void DrawDoubleBorder(bool doubleBorder, SizeF inSize)
        {
            _doubleBorder = doubleBorder;
            _inSize = inSize;
        }

        private void DrawDoubleBround(Graphics g)
        {
            if (_doubleBorder)
            {
                Size clientSize = _canvas.Container.ClientSize;
                g.FillRectangle(new SolidBrush(Color.White), new RectangleF(new PointF(0, 0), new SizeF(clientSize.Width, _inSize.Height)));
                g.FillRectangle(new SolidBrush(Color.White), new RectangleF(new PointF(0, clientSize.Height - _inSize.Height), new SizeF(clientSize.Width, _inSize.Height)));
                g.FillRectangle(new SolidBrush(Color.White), new RectangleF(new PointF(0, 0), new SizeF(_inSize.Width, clientSize.Height)));
                g.FillRectangle(new SolidBrush(Color.White), new RectangleF(new PointF(clientSize.Width - _inSize.Width, 0), new SizeF(_inSize.Width, clientSize.Height)));
                g.DrawRectangle(new Pen(Color.Black), _inSize.Width, _inSize.Height, clientSize.Width - 2 * _inSize.Width, clientSize.Height - 2 * _inSize.Height);
            }
        }

        private void ComputeCurrentScreRectange()
        {
            _curViewScrRect = _canvas.Container.ClientRectangle;
        }
            ...
            try
            {
                ...
                for (int iLine = 0; iLine < endLon; iLine += _lonLabelStep)
                {
                    ...
                    if (!_enableMaskColor || _maskBrush == null)
                        ...
                        DrawStringWithBorder(labelGeoX, g, pt, _labelFont, _labelBrush, _maskBrush, null);
                }
                //label latlines
                firstLine = -1;
                ...
                for (int iLine = begin - 1; iLine > endLat; iLine -= _latLabelStep)
                {
                    ...
                    if (!_enableMaskColor || _maskBrush == null)
                        ...
                        DrawStringWithBorder(labelGeoY, g, pt, _labelFont, _labelBrush, _maskBrush, _stringFormat);
                }
            }
            catch (Exception ex)
            {
        ...
        //added by wangyu 修改标注显示位置
        private void DrawLabel(Graphics g)
        {
            //如果经纬度网格线与边线有交点，则在上下/左右交点处进行标注；
            //如果经纬度网格线与边线没有交点（经纬度网格全范围显示的情况），则按照原来的点进行标注，
            //或者不标注（经纬度网格全部不显示）                    
            if (!_enableLabling)
                return;
            ...
            try
            {
                //每条线标注两个端点，或者不标注
                //label lonlines
                DrawLonLineLabels(g, ptBegin, ptEnd);
                //label latlines
                DrawLatLineLabels(g, ptBegin, ptEnd);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message + ptBegin.X + "," + ptBegin.Y + "," + ptEnd.X + "," + ptEnd.Y);
            }
        }

        //标注经度线
        private void DrawLonLineLabels(Graphics g, PointF ptBegin, PointF ptEnd)
        {
            int idx = -1;
            string labelGeoX = string.Empty, labelGeoY = string.Empty;
            int firstLine = -1;
            ...
            for (int iLine = 0; iLine < endLon; iLine += _lonLabelStep)
            {
                if (firstLine == -1)
                    firstLine = iLine;
                if (!ComputeLineIntersectionPoints(_gridLines[iLine], out ptBegin, out ptEnd))
                    //所有顶点都在屏幕外面，不进行标注
                    continue;
                //取当前经度线上任意一个点，计算其经纬度坐标，并使用其经度坐标进行标注                
                idx = _gridLines[iLine].BeginIndex + 1;
                geoX = _allPrjPoints[idx].X;
                geoY = _allPrjPoints[idx].Y;
                _coordTransfrom.Prj2Geo(geoX, geoY, out geoX, out geoY);
                if (double.IsInfinity(geoX) || double.IsNaN(geoX) || double.IsInfinity(geoY) || double.IsNaN(geoY))
                ...
                if (iLine == firstLine)
                {
                    ptBegin.X += _fontSize.Height;
                    ptEnd.X += _fontSize.Height;
                }
                //by chennan 修改为当前标注实际范围
                _fontSize.Height = g.MeasureString(labelGeoX, _labelFont).Height;
                //避免因Y==container.Height，而出现绘制在无法显示的区域问题
                if ((_canvas.Container.Height - ptBegin.Y) < _fontSize.Height)
                    ptBegin.Y -= _fontSize.Height;
                if ((_canvas.Container.Height - ptEnd.Y) < _fontSize.Height)
                    ptEnd.Y -= _fontSize.Height;
                if (!_enableMaskColor || _maskBrush == null)
                {
                    //经度线不能在左右边线上进行标注
                    if (ptBegin.X > 0 && ptBegin.X != _canvas.Container.Width)
                        g.DrawString(labelGeoX, _labelFont, _labelBrush, ptBegin);
                    if (ptEnd.X > 0 && ptEnd.X != _canvas.Container.Width)
                        g.DrawString(labelGeoX, _labelFont, _labelBrush, ptEnd);
                }
                else
                {
                    if (ptBegin.X > 0 && ptBegin.X != _canvas.Container.Width)
                        DrawStringWithBorder(labelGeoX, g, ptBegin, _labelFont, _labelBrush, _maskBrush, null);
                    if (ptEnd.X > 0 && ptEnd.X != _canvas.Container.Width)
                        DrawStringWithBorder(labelGeoX, g, ptEnd, _labelFont, _labelBrush, _maskBrush, null);
                }
            }
        }

        //标注纬度线
        private void DrawLatLineLabels(Graphics g, PointF ptBegin, PointF ptEnd)
        {
            int idx = -1;
            string labelGeoX = string.Empty, labelGeoY = string.Empty;
            int firstLine = -1;
            ...
            for (int iLine = begin - 1; iLine > endLat; iLine -= _latLabelStep)
            {
                ...
                //by chennan 修改为当前标注实际范围
                _fontSize.Width =_doubleBorder? g.MeasureString(labelGeoY, _labelFont).Height:g.MeasureString(labelGeoY, _labelFont).Width;
                //避免因Y==container.Height，而出现绘制在无法显示的区域问题
                if ((_canvas.Container.Width - ptBegin.X) < _fontSize.Width)
                    ptBegin.X -= (_fontSize.Width);
                if ((_canvas.Container.Width - ptEnd.X) < _fontSize.Width)
                    ptEnd.X -= (_fontSize.Width);
                if (!_enableMaskColor || _maskBrush == null)
                {
                    //纬度线不能在上下边线上进行标注
                    if (ptBegin.Y > 0 && ptBegin.Y != _canvas.Container.Height)
                        g.DrawString(labelGeoY, _labelFont, _labelBrush, ptBegin, _doubleBorder ? _stringFormat : null);
                    if (ptEnd.Y > 0 && ptEnd.Y != _canvas.Container.Height)
                        g.DrawString(labelGeoY, _labelFont, _labelBrush, ptEnd, _doubleBorder ? _stringFormat : null);
                }
                else
                {
                    if (ptBegin.Y > 0 && ptBegin.Y != _canvas.Container.Height)
                        DrawStringWithBorder(labelGeoY, g, ptBegin, _labelFont, _labelBrush, _maskBrush, _doubleBorder ? _stringFormat : null);
                    if (ptEnd.Y > 0 && ptEnd.Y != _canvas.Container.Height)
                        DrawStringWithBorder(labelGeoY, g, ptEnd, _labelFont, _labelBrush, _maskBrush, _doubleBorder ? _stringFormat : null);
                }
            }
        }
        ...
        /// <returns></returns>
        private unsafe bool ComputeLineIntersectionPoints(GridLine gridLine, out PointF ptBegin, out PointF ptEnd)
        {
            int count = 0;
            int length = 0;
            PointF[][] outPoints = null;
            ptBegin = ptEnd = PointF.Empty;
            fixed (PointF* ptr0 = _allPixelPoints)
            {
                ...
                int prePointCount = 0;
                int ptCount = gridLine.SegmentCount;
                ...
                fixed (PointF* buffer0 = vertexBuffer)
                {
                    ...
                    allPointPtr = ptr0 + gridLine.BeginIndex;
                ...
                PolyLineSegmentor.SplitWithClipBounds(vertexBuffer, _curViewScrRect, out outPoints);
            }
            //所有顶点都在屏幕外面
            if (outPoints == null || outPoints.Length == 0)
                return false;
            count = outPoints.Length;
            if (outPoints[count - 1] == null || outPoints[count - 1].Length == 0)
                return false;
            length = outPoints[count - 1].Length;
            //取第一条线的第一个点和最后一条线的最后一个点
            ptBegin = outPoints[0][0];
            ptEnd = outPoints[count - 1][length - 1];
            return true;
        }
            ...
            try
            {
                fixed (PointF* ptr0 = _allPixelPoints)
                {
                    ...
                    for (int iLine = beginLine; iLine < endLine; iLine++)
                    {
                        ...
                        //   
                        //切割经纬度网格，只绘制在视图范围内的线段
                        RectangleF tempView = _doubleBorder ? ResetRect(_curViewScrRect) : _curViewScrRect;
                        PolyLineSegmentor.SplitWithClipBounds(vertexBuffer, tempView, out outPoints);
                        if (outPoints != null && outPoints.Length != 0)
                        {
                            int length = outPoints.Length;
                            for (int i = 0; i < outPoints.Length; i++)
                                g.DrawCurve(_gridPen, outPoints[i]); //vertexBuffer);
                        }
                    }
                }
            }
        ...
        private RectangleF ResetRect(RectangleF _curViewScrRect)
        {
            return new RectangleF(new PointF(_inSize.Width, _inSize.Height), new SizeF(_curViewScrRect.Width - _inSize.Width * 2, _curViewScrRect.Height - _inSize.Height * 2));
        }
        ...
        public void DrawStringWithBorder(string text, Graphics g, PointF pointF, Font font, Brush fontBrush, Brush maskBrush, StringFormat stringFormat)
        {
            g.DrawString(text, font, maskBrush, pointF.X, pointF.Y, stringFormat);
            g.DrawString(text, font, maskBrush, pointF.X + 1, pointF.Y - 1, stringFormat);
            g.DrawString(text, font, maskBrush, pointF.X + 1, pointF.Y, stringFormat);
            g.DrawString(text, font, maskBrush, pointF.X - 1, pointF.Y + 1, stringFormat);
            g.DrawString(text, font, maskBrush, pointF.X - 1, pointF.Y - 1, stringFormat);
            g.DrawString(text, font, maskBrush, pointF.X - 1, pointF.Y, stringFormat);
            g.DrawString(text, font, maskBrush, pointF.X, pointF.Y + 1, stringFormat);
            g.DrawString(text, font, maskBrush, pointF.X, pointF.Y - 1, stringFormat);
            //
            g.DrawString(text, font, fontBrush, pointF.X, pointF.Y, stringFormat);
        }
        #region Xml
