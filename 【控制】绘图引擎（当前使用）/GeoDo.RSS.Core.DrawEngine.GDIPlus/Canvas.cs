...
namespace GeoDo.RSS.Core.DrawEngine.GDIPlus
{
        ...
        IControlMessageAccepter, IReversedCoordinateTransform
    {
        ...
        protected const float ZOOM_MIN_PERCENT = 0.001f;
        ...
        private void AttachEvents()
        {
            ...
            _container.PreviewKeyDown += new PreviewKeyDownEventHandler(_container_PreviewKeyDown);
        }

        void _container_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            DrawingMouseEventArgs arg = new DrawingMouseEventArgs();
            arg.E = e;
            if (_currentControlLayer != null)
                (_currentControlLayer as ICanvasEvent).Event(this, enumCanvasEventType.KeyDown, arg);
        }
        ...
        private void DrawScalePercent(Graphics g)
        {
            ...
            double scale = 0;
            if (_primaryDrawObject != null)
                scale = Scale * 100f * _primaryDrawObject.OriginalResolutionX;
            else
                scale = Scale * 100f;
            if (scale <= 10)
        ...
        private void ComputeScaleAndOffsets()
        {
            ...
            if (_isFitWidth)//全屏显示，按宽度适应，显示最上面
            {
                _scale = scaleX;
            }
            else
                _scale = Math.Min(scaleX, scaleY);
            if (_primaryDrawObject != null)//限制最大、最小比例尺
            {
            ...
            if (_isFitWidth)//全屏显示，按宽度适应，上对齐
            {
                if (_isReverseDirection)
                    _offsetY = -(float)(_currentEnvelope.MinY + _container.Height / _scale);
                else
                    _offsetY = -(float)_currentEnvelope.MaxY;
            }
            else
