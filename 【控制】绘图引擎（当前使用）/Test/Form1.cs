...
namespace Test
{
    public partial class Form1 : Form, IPerformanceWatch, ILoadingPrecentSubscriber
    {
        ...
        private void btnCreateRasterDrawing_Click(object sender, EventArgs e)
        {
            ...
            drawing.SelectedBandNos = new int[] { 3, 4, 2 };
        ...
        private void canvasHost1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            //Text = string.Format("geoX:{0},geoY:{1}", rasterX.ToString(), rasterY.ToString());
            //
            //_canvas.CoordTransform.QuickTransform.Transform(ref prjX, ref prjY);
            //Text = "from {" + prjX.ToString() + "," + prjY.ToString() + "}";
            //_canvas.CoordTransform.QuickTransform.InverTransform(ref prjX, ref prjY);
        ...
        private void button27_Click(object sender, EventArgs e)
        {
            ...
            string fname = @"F:\产品与项目\MAS-II\SMART0718\SMART\【控制】UI框架(新整理)\SMART\bin\Release\数据引用\基础矢量\矢量模版\海陆模版.shp";
        ...
        private void button32_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog dlg = new OpenFileDialog())
            {
                if (dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    MemEstimatorOfRasterDrawing mem = new MemEstimatorOfRasterDrawing();
                    int memoryOfTile ,tileCount;
                    float memSize = mem.Estimate(dlg.FileName,out memoryOfTile,out tileCount);
                    MessageBox.Show("估计需要内存：" + memSize.ToString() + " MB");
                }
            }
        }
