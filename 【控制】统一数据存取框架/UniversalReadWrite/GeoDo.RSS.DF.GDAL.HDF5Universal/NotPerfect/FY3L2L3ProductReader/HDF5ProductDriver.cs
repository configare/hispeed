...
namespace GeoDo.RSS.DF.GDAL.HDF5Universal
{
    /// <summary>
    /// 类名：HDF5Driver
    /// 属性描述：依据已配置定义的风云三号卫星HDF产品定义，来自动识别风三产品数据、数据集，自动调用然后调用HDF5通用读取模块，加载读取数据。
    /// 创建者：罗战克   创建日期：2013-08-16 15:38:44
    /// 修改者：             修改日期：
    /// 修改描述：
    /// 备注：
    /// </summary>
    ...
    //本驱动尚未完善
    public class HDF5ProductDriver : RasterDataDriver
    {
        public HDF5ProductDriver()
            :base()
        {
            _name = "HDF5Product";
        }
