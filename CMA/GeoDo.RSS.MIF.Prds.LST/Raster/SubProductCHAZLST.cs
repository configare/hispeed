...
{
    public class SubProductCHAZLST : CmaMonitoringSubProduct
    {
        private IContextMessage _contextMessage = null;

        public SubProductCHAZLST()
            : base()
        {

        }

        public SubProductCHAZLST(SubProductDef subProductDef)
            : base(subProductDef)
        {
        ...
        private IExtractResult CalcAnmi(Action<int, string> progressTracker)
        {
            ...
            try
            {
                ...
                //输出文件准备（作为输入栅格并集处理）
                RasterIdentify ri = GetRasterIdentifyID(lstFile, avg);
