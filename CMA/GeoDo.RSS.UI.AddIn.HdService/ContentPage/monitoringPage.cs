...
namespace GeoDo.RSS.UI.AddIn.HdService
{
    public partial class monitoringPage : UserControl
    {
                    ...
                    else if (arg2[0] is HdDataServer.BlockInfo)
                    {
                        for (int i = 0; i < arg2.Length; i++)
                        {
                            ...
                            if (info.mosaicInfo != null && info.mosaicInfo.dayOrNight != "D")
                                continue;
                            if (info.mosaicInfo == null && info.projectionInfo.orbitInfo.dayOrNight != "D")
                                continue;
        ...
        private void ShowLocationMenu(double lon, double lat, Point mousePoint)
        {
            ...
            if (blockItems != null && blockItems.Length != 0)
            {
                ...
                foreach (BlockInfoItem block in items)
                {
                    ...
                    if (block.BlockInfo.mosaicInfo != null && block.BlockInfo.mosaicInfo.dayOrNight != "D")
                        continue;
                    if (block.BlockInfo.mosaicInfo == null && block.BlockInfo.projectionInfo.orbitInfo.dayOrNight != "D")
                        continue;
            ...
            ToolStripMenuItem MosaicBlockMenu = new ToolStripMenuItem();
            int MosaicBlockCount = mosaicCount == 0 ? pjCount : mosaicCount;
            if (MosaicBlockCount != 0)
            {
                if (mosaicCount == 0)
                    CreatMosaicBlock(projectionMenu.DropDownItems, MosaicBlockMenu);
                else
                    CreatMosaicBlock(mosaicMenu.DropDownItems, MosaicBlockMenu);
            }
            ...
            menu.Items.Add(new ToolStripSeparator());
            MosaicBlockMenu.Text = "轨道分幅数据(" + MosaicBlockCount + ")";
            menu.Items.Add(MosaicBlockMenu);
        ...
        private void CreatMosaicBlock(ToolStripItemCollection toolStripItemCollection, ToolStripMenuItem MosaicBlockMenu)
        {
            List<ToolStripItem> tools = new List<ToolStripItem>();
            foreach (ToolStripItem item in toolStripItemCollection)
            {
                ToolStripMenuItem tool = new ToolStripMenuItem(item.Text);
                tool.Tag = item;
                tool.Click += new EventHandler(toolBlock_Click);
                tools.Add(tool);
            }
            MosaicBlockMenu.DropDownItems.AddRange(tools.ToArray());
        }
