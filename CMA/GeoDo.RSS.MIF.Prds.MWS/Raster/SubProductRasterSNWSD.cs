    ...
    public class SubProductRasterSNWSD : CmaMonitoringSubProduct
    {
            ...
            try
            {
                ...
                rfr.RegisterCalcModel(new RasterCalcHandler<Int16, float>((rvInVistor, rvOutVistor, aoi) =>
                {
                    ...
                    for (int i = 0; i < dataLength; i++)
                    {
                        ...
                        float invlogpol10 = Convert.ToSingle(1 / Math.Log10((double)po110));
                        float invlogpol18 = Convert.ToSingle(1.0 / Math.Log10((double)po118));
                        float invlogpol36 = Convert.ToSingle(1.0 / Math.Log10((double)po136));
                        int flag = 0;
                        if ((ch36v < 255) && (ch36h < 245) && (t1036v > 0))
                        {
                            flag = 1;
                        }
                        if ((ch36v < 255) && (ch36h < 245) && (flag == 0) && (t1036h > 0))
                        {
                            flag = 2;
                        }
                        if ((ch89v < 255) && (ch89h < 255) && (t2389v > 0) && (t2389h > 0) && (flag == 0) && (klvn < 267))
                        {
                            flag = 3;
                        }

                        if (flag == 1)
                            sdtmp[i] = ff * (invlogpol36 * t1836v) / (1 - scl) + ((1 - ff) * ((invlogpol36 * t1036v) + (invlogpol18 * t1018v)));
                        if (flag == 2)
                            sdtmp[i] = (ff * (invlogpol36 * t1836h) / (1 - scl)) + ((1 - ff) * ((invlogpol36 * t1036h) + (invlogpol18 * t1018h)));
                        if (flag == 3)
                            sdtmp[i] = 5.0f;
                        if (sdtmp[i] < 5 && sdtmp[i] > 0)
                            sdtmp[i] = 5.0f;
        ...
        private ExtractResultArray GetWholeSDAndSWE(List<string> inputfiles)
        {
            ...
            try
            {
                ...
                {
                    for (int i = 0; i < rvOutVistor[0].RasterBandsData[0].Length; i++)
                    {
                    ...
                    for (int i = 0; i < dataLength; i++)
                    {
                        if (rvInVistor[2].RasterBandsData[0] != null)
                        {
                            ...
                            {
                                rvOutVistor[0].RasterBandsData[0][i] = rvInVistor[0].RasterBandsData[0] != null ? rvInVistor[0].RasterBandsData[0][i] : -999.0f;
                                rvOutVistor[1].RasterBandsData[0][i] = rvInVistor[1].RasterBandsData[0] != null ? rvInVistor[1].RasterBandsData[0][i] : -999.0f;
                            }
                            else
                            {
            ...
            try
            {
                ...
                rfr.RegisterCalcModel(new RasterCalcHandler<Int16, float>((rvInVistor, rvOutVistor, aoi) =>
                {
                        ...
                        rvInVistor[4].RasterBandsData[0] != null)
                    {
                        ...
                        int[] type = new int[dataLength];
                        float[] sdtmp = new float[dataLength];
                        if (rvInVistor[5].RasterBandsData[0] != null)
                        {
                            #region 判识算法细节
                            for (int i = 0; i < dataLength; i++)
                            {
