...
namespace GeoDo.RSS.MIF.Prds.DRT
{
    ...
    /// </summary>
    public partial class UCFilesSelectorDRT : UserControl, IArgumentEditorUI2
    {
        ...
        private string _saveFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "MonitoringProductArgs", "DRT", "局地文件夹路径.txt");

        public UCFilesSelectorDRT()
        {
            InitializeComponent();
            InitArgument();
        ...
        private void robCurrent_CheckedChanged(object sender, EventArgs e)
        {
            if (_handle != null)
                _handle(GetArgument());
        }
