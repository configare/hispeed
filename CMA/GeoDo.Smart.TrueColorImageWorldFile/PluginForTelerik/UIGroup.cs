...
namespace GeoDo.Smart.TrueColorImageWorldFile
{
    public partial class UIGroup : UserControl, IUIGroupProvider
    {
        ...
        RadButtonElement _btnCompute = new RadButtonElement("FY3真彩图配准");
        ...
        void _btnCompute_Click(object sender, EventArgs e)
        {
            try
            {
                string exe = System.AppDomain.CurrentDomain.BaseDirectory + "GeoDo.Smart.TrueColorImageWorldFile.exe";
                Process.Start(exe);
            }
            catch (Exception ex)
            {
        ...
        private void SetImage()
        {
            if (_session != null)
            {
                _btnCompute.Image = _session.UIFrameworkHelper.GetImage("cma:TureColor.png");
            }
        }
    }
}
