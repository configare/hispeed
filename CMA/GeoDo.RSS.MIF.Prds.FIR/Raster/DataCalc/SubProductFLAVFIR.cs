...
{
    public class SubProductFLAVFIR : CmaMonitoringSubProduct
    {
        private IContextMessage _contextMessage = null;

        public SubProductFLAVFIR()
            : base()
        {
        }

        public SubProductFLAVFIR(SubProductDef subProductDef)
            : base(subProductDef)
        {
        ...
        public override IExtractResult Make(Action<int, string> progressTracker, IContextMessage contextMessage)
        {
            ...
            try
            {
                for (int i = 0; i < fileNames.Length; i++)
                {
                    ...
                    {
                        PrintInfo("请选择正确的数据进行平均值合成,请检查文件[" + fileNames[i] + "].");
                        return null;
                    }
