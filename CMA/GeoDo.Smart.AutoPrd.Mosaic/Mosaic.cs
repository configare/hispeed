...
namespace GeoDo.Smart.AutoPrd.Mosaic
{
    ...
    public class MosaicSplice
    {
        ...
        public void DoMosaicSplice()
        {
            ...
            {
                if (string.IsNullOrEmpty(inArg.MosaicIdentify))
                    MosaicToFile();
                else
                    AngleMosaicToFile();
            }
        ...
        private string GetFirstname(string fileOrDir)
        {
            if (fileOrDir.Contains("|*"))
            {
                ...
                if (Directory.Exists(fileInfo[0]))
                {
                    ...
                    return files == null || files.Length == 0 ? fileOrDir : files[0];
                }
                return fileOrDir;
            }
            else
        ...
        private string GetGeoHeadInfo(PrjEnvelopeItem prjEnvelopeItem)
        {
            return string.Format("{0},{1},{2},{3},{4},{5}", inArg.MosaicResoultionX, inArg.MosaicResoultionY, prjEnvelopeItem.PrjEnvelope.MinX, prjEnvelopeItem.PrjEnvelope.MaxY,
                (int)(prjEnvelopeItem.PrjEnvelope.Width / inArg.MosaicResoultionX), (int)(prjEnvelopeItem.PrjEnvelope.Height / inArg.MosaicResoultionY));
        }
