...
namespace GeoDo.RSS.MIF.Prds.UHE
{
    public class SubProductBinaryLMCZ : CmaMonitoringSubProduct
    {
        ...
        private IExtractResult ComputeByPrdFiles(Action<int, string> progressTracker)
        {
            ...
            IExtractResultArray arrary = new ExtractResultArray("UHE");
            float step = 100 / length;
            for (int i = 0; i < length; i++)
            {
                if (progressTracker != null)
                    progressTracker.Invoke((int)Math.Floor(i * step), "正在计算,请稍后...");

                aoiVaildValueCount = 0;
                sumAOILstValue = 0;
                for (int index = 0; index < minValueLength; index++)
                    minLstValue[index] = UInt16.MaxValue;
                ...
                try
                {
                    if (string.IsNullOrEmpty(cloudFile))
                    {
                        ...
                        if (!File.Exists(cloudFile) || cloudFile == fileNames[i])
                    ...
                    rpVisitor.VisitPixel(new int[] { LstBandCH },
                     (index, values) =>
                     {
                         ...
                         {
                             tempLst = (UInt16)(values[0] / LstBandZoom * LMCZZoom);
                             UpdateMinValueArrary(ref minLstValue, tempLst, LSTMin, LMCZZoom, true);
                             if (aoi != null && aoi.Contains(index))
                             {
                                 sumAOILstValue += tempLst;
                                 aoiVaildValueCount++;
                             }
                             resultTemp.Put(index, tempLst);
                         }
                     });
            ...
            return arrary.PixelMappers.Length == 0 ? null : arrary; ;
        }
        ...
        private IExtractResult ComputeByCurrentRaster(IRasterDataProvider currPrd, Action<int, string> progressTracker)
        {
            ...
            if (isAppCloud && !string.IsNullOrEmpty(cloudFile) && File.Exists(cloudFile))
                UpdateAOI(ref aoi, currPrd, cloudFile, out cloudIndex);
            //
             ...
             (index, values) =>
             {
                 ...
                 if (values[0] == 0)
                     resultTemp.Put(index, 0);
                          ...
                          isAutoCloud && values[1] / NearInfraredZoom > NearInfraredCLMMin && values[3] / FarInfrared11Zoom < FarInfrared11CLMMax && Math.Abs(values[2] / MiddInfraredZoom - values[3] / FarInfrared11Zoom) > FarInfrared1112CLMMin)
                     resultTemp.Put(index, defCloudy);
                 else
                 {
                        ...
                        if (rvInVistor[0].RasterBandsData[0] != null && rvInVistor[1].RasterBandsData[0] != null)
                        {
                            if (resultAOI.Count != 0)
                                for (int index = 0; index < dataLength; index++)
                                {
                                    ...
                                    if (data1 != 0)
                                    {
                                        cloudIndexList.Add(blockLength + index);
                                        if (resultAOI.Contains(blockLength + index))
                                            resultAOI.Remove(blockLength + index);
                                        continue;
                                    }
                                }
                            else
