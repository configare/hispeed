...
namespace GeoDo.RSS.MIF.Prds.DST
{
    ...
    /// 沙尘对比分析
    /// </summary>
    /// 重构思路：这里用了几个子方法 具有公用性，显然如果放在基类里面比较好些
    public class SubProductCompDST : CmaMonitoringSubProduct
    {
        public SubProductCompDST(SubProductDef subProductDef)
            : base(subProductDef)
        {
        }
        ...
        public override IExtractResult Make(Action<int, string> progressTracker)
        {
            ...
            if (_argumentProvider.GetArg("AlgorithmName") == null)
                return null;
            if (_argumentProvider.GetArg("AlgorithmName").ToString() == "COMD")
            {
