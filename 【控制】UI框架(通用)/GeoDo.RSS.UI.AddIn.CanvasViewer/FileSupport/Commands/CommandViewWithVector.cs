...
namespace GeoDo.RSS.UI.AddIn.CanvasViewer
{
    ...
    public class CommandViewWithVector : Command
    {
        public CommandViewWithVector()
            : base()
        {
            _id = 2002;
            _text = _name = "另存为图片(原始影像+矢量)";
        }
                ...
                using (SaveFileDialog diag = new SaveFileDialog())
                {
                    ...
                    if (diag.ShowDialog() == DialogResult.OK)
                    {
                        try
                        {
                            ...
                            {
                                using (Bitmap img = GetRasterBitmap(rd))
                                {
                                    DrawVectorToBitmap(canViewer, img);
        ...
        private void DrawVectorToBitmap(ICanvasViewer canViewer, Bitmap img)
        {
            using (Graphics g = Graphics.FromImage(img))//矢量绘制到bmp上
            {
                using (Bitmap bmpVector = canViewer.Canvas.GetBitmapUseOriResolution())
                {
                    g.DrawImage(bmpVector, 0, 0);
                }
            }
        }

        private Bitmap GetRasterBitmap(IRasterDrawing rd)
        {
            Bitmap img = rd.GetBitmapUseOriginResolution();
            {
