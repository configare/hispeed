...
{
    internal class ContourGeneratorByte : ContourGenerator<byte>
    {
        protected override byte GetNoDataValueOutsideAOI()
        {
            return (byte)_noData;
        }

        protected override bool CheckIsContained(byte v1, byte v2, byte val)
        {
            if (v1 == v2)
                return false;
            if (v1 == val)
        ...
        protected override byte ToTValue(double v)
        {
            return (byte)v;
        }
        ...
        protected override PointF GetVPoint(enumEdgeType edgeType, int pixelIdxV1, int pixelIdxV2, double contValue)
        {
            ...
            float y = pixelIdxV1 / _width;//row
            float x = pixelIdxV1 % _width;//col
            switch (edgeType)
            {
                case enumEdgeType.A:
